...
#   as.data.frame()%>%
#   select(c("seqnames", "start", "end", "enhancer", "correlation", "FDR"))%>%
#   separate(col=`enhancer`, into= c("seqnames2", "start2", "end2"))%>%tibble-> InteractionDataFrameCAGEExpression
# 
...
#                               "Interaction",
#                               InteractionDataFrameCAGEExpression$correlation*10)
# 
...
#             )
# EnhancerAndCAGEPRomoterInteraction<-makeGenomicInteractionsFromFile(fn = "/media/awais/NewDrivewho/UnbiasedTFBSPrediction/Unbiased TFBS Prediction/DataFiles/Interactions/Human/EnhancerPromoterAssoication.bedpe", type = "bedpe")
# 
 mcols(EnhancerAndCAGEPRomoterInteraction)<-cbind.data.frame(mcols(EnhancerAndCAGEPRomoterInteraction), "FDR" = InteractionDataFrameCAGEExpression$FDR)
 
 saveRDS(object = EnhancerAndCAGEPRomoterInteraction, "DataFiles/Interactions/Human/EnhancerPromoterAssoicationRObject")
# 
